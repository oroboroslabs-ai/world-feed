...
(function () {
    ...
    const filters = document.getElementById('wf-filters');
    const search = document.getElementById('wf-search-input');
    const feedTitle = document.getElementById('wf-feed-title');
    ...
    function loadPrecogFeed() {
        fetch('data/worldfeed.json?t=' + Date.now())
            .then(r => r.json())
            .then(data => {
                if (data.stories && data.stories.length > 0) {
                    console.log('Loading Precog stories:', data.stories.length);
                    // Normalize and validate Precog stories
                    const precogStories = validateFeedItems(data.stories);
                    console.log('Validated Precog stories:', precogStories.length);
                    
                    // Add Precog stories at the beginning with higher priority
                    const enhancedPrecog = precogStories.map(s => ({
                        ...s,
                        tier: Math.max(s.tier, 5), // Ensure Precog stories have high tier
                        cats: [...(s.cats || []), 'precog', 'verified']
                    }));
                    
                    // Merge with existing posts
                    basePosts = [...enhancedPrecog, ...basePosts];
                    allPosts = mergeFeeds(basePosts);
                    ...
                    updateHero();
                    updateWatchList();
                    console.log('Precog feed loaded successfully - Total posts:', allPosts.length);
                }
            })
            .catch(err => {
                console.log('No Precog feed available, using seed data:', err.message);
            });
    }
